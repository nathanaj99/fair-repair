...
def patchTree(evalu, cols, refineHeuristics, attr_map,classes):
    ...
    if sat:
        ...
        tree_str = repaired_tree_to_code(classifier,features,hsets,hCubeSetIndices,refineHeuristics,0)
        evalu.set_tree(tree_str)
        evalu.save_tree()
        return tree_str
    ...
    if refine_result != None:
        ...
        tree_str = repaired_tree_to_code(classifier,features,hsets,hCubeSetIndices,refineHeuristics,totalNumRef)
        evalu.set_tree(tree_str)
        evalu.save_tree()
...
def repaired_tree_to_code(tree,feature_names,hsets,sens_groups,refineHeuristics,totalNumRef):
    ...
    # Feature_name is just a list of the features that the classifier branches on. If the feature is in the data
    # but not used by the tree, then it's undefined. position in list corresponds to node

    hid = 1

    def recurse(node, depth, t_str, hid):
        indent = "  " * depth * 2
        if tree_.feature[node] != _tree.TREE_UNDEFINED: # means you branch
            name = feature_name[node]
            threshold = tree_.threshold[node]
            t_str += "{}if {} <= {}:".format(indent, name, threshold) + "\n"
