...
def create_base_parser(description, sensitive_attrs_default, dataset_train_default, dataset_test_default):
    ...
    parser.add_argument('--tree-output', type=str,
                        help='Filename where to write out the tree output (default: stdout)',
                        default=None,
                        required=False)
    # Dataset Train arg:
    parser.add_argument('-i', '--dataset_train', type=str,
                        help='Choice of training dataset (or subset)',
                        default=dataset_train_default,
                        required=False)
    # Dataset Test arg:
...
class EvalUtil:
    ...
    def get_dataset_train(self):
        return self.args.dataset_train
    ...
    def save_tree(self):
        fname = self.args.tree_output
        if fname is not None:
            outf = open(fname, 'w')
            outf.write(self.tree)
            outf.close()
        return
