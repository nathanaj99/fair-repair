...
def patchTree(evalu, cols, refineHeuristics, attr_map,classes):
    ...
    ds_train = evalu.get_dataset_train()
    len_ds_train,df_train,X_train,y_train = readData(ds_train, cols)
    features = list(X_train.columns)

    # Testing set Gathering
    # Added by Jack
    ...
    last_time = time.time()
    classifier, y_pred = trainDecisionTree(X_train,y_train,evalu.get_seed())

    evalu.record_eval("time-tree-construction", time.time() - last_time)

    # For the training set

    # Datalist is the training set
    sizeOfDataset = len_ds_train
    to_dict_start = time.time()
    dataList = []
    for i in range(len(df_train)):
        dataList.append(df_train.iloc[i].to_dict())
        dataList[i]['index'] = i
    ...
    # Record outcomes and compute accuracy for training set
    classify_start = time.time()
    predicted_outcomes = [classifier.predict(X_train.iloc[[i]])[0] for i in range(len(df_train))]
    actual_outcomes = [dataList[i]['Class'] for i in range(len(df_train))]

    true_pos, false_pos, true_neg, false_neg = accuracyCalculator(predicted_outcomes,actual_outcomes,cls1,cls2,dataList)

    evalu.record_eval("classification-time train", time.time()-classify_start)
    evalu.record_eval("true-positive-before train", true_pos)
    evalu.record_eval("false-positive-before train", false_pos)
    evalu.record_eval("true-negative-before train", true_neg)
    evalu.record_eval("false-negative-before train", false_neg)
    evalu.record_eval("precision-before train",true_pos/(true_pos+false_pos))
    evalu.record_eval("recall-before train",true_pos/(true_pos+false_neg))
    evalu.record_eval("accuracy-before train",(true_neg+true_pos)/(true_pos+true_neg+false_pos+false_neg))

    # Record outcomes and compute accuracy for testing set

    # Added by Jack
    predicted_outcomes_test = [classifier.predict(X_test.iloc[[i]])[0] for i in range(len(df_test))]
    actual_outcomes_test = [dataList_test[i]['Class'] for i in range(len(df_test))]

    true_pos_test, false_pos_test, true_neg_test, false_neg_test = accuracyCalculator(predicted_outcomes_test,actual_outcomes_test,cls1,cls2,dataList_test)

    evalu.record_eval("classification-time test", time.time()-classify_start)
    evalu.record_eval("true-positive-before test", true_pos)
    evalu.record_eval("false-positive-before test", false_pos)
    evalu.record_eval("true-negative-before test", true_neg)
    evalu.record_eval("false-negative-before test", false_neg)
    evalu.record_eval("precision-before test",true_pos/(true_pos+false_pos))
    evalu.record_eval("recall-before test",true_pos/(true_pos+false_neg))
    evalu.record_eval("accuracy-before test",(true_neg+true_pos)/(true_pos+true_neg+false_pos+false_neg))
    ...
    # Modified by Jack so it only looks at training
    evalu.record_eval("data-file", evalu.get_dataset_train())
    evalu.record_eval("num-input-points", len_ds_train)
    ...
    # cols used for categorical variable dummies
    cols = [
    'race',
    'age_cat',
    'sex',
    ...
    # Importing test dataset and organizing it in such a way where we can predict it
    df_test_for_pred = df_test.drop(['frequency','Class'],axis=1)
    cols_for_pred = df_test_for_pred.columns
    ds_test_2 = pd.read_csv('./data/'+ds_test,index_col=None,sep=',')
    df_test_encoded_pre = pd.get_dummies(ds_test_2, columns = cols)
    df_test_encoded = df_test_encoded_pre.drop("Class",axis=1)
    print(df_test_encoded.columns)

    # Actual labels from test set
    actual_test_outcomes = df_test_encoded_pre['Class']


    # Predictions using the tree defined below
    test_predictions = []
    for i in np.arange(len_ds_test):
        pred = compas_tree(df_test_encoded.iloc[i,0],df_test_encoded.iloc[i,1],df_test_encoded.iloc[i,2],df_test_encoded.iloc[i,3],
            df_test_encoded.iloc[i,4],df_test_encoded.iloc[i,5],df_test_encoded.iloc[i,6],df_test_encoded.iloc[i,7],df_test_encoded.iloc[i,8],
            df_test_encoded.iloc[i,9],df_test_encoded.iloc[i,10],df_test_encoded.iloc[i,11],df_test_encoded.iloc[i,12],df_test_encoded.iloc[i,13],
            df_test_encoded.iloc[i,14],df_test_encoded.iloc[i,15],df_test_encoded.iloc[i,16],df_test_encoded.iloc[i,17],df_test_encoded.iloc[i,18],
            df_test_encoded.iloc[i,19])
        test_predictions.append(pred)

    # The tree outputs binary 0-1, so we will change them to binary 1-2 which matches our dataset
    for i in np.arange(len(test_predictions)):
        test_predictions[i] = test_predictions[i]+1

    from sklearn.metrics import accuracy_score
    print(accuracy_score(actual_test_outcomes,test_predictions))


    # Jack ends adding here # 
...
def repaired_tree_to_code(tree,feature_names,hsets,sens_groups,refineHeuristics,totalNumRef):
    ...
    def recurse(node, depth, t_str, hid):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            t_str += "\n{}return {}".format(indent,np.argmax(li)) + "\n"
        return t_str
...
def compas_tree(priors_count, race_1, race_2, race_3, race_4, age_cat_1, age_cat_2, age_cat_3, sex_1, sex_2, c_charge_degree_1, c_charge_degree_2, length_of_stay_1, length_of_stay_2, length_of_stay_3, length_of_stay_4, length_of_stay_5, score_text_1, score_text_2, score_text_3):
    if score_text_3 <= 0.5:
        if priors_count <= 2.5:
            if sex_1 <= 0.5:
                if age_cat_3 <= 0.5:
                    if priors_count <= 1.5:
                        if length_of_stay_2 <= 0.5:
                            if race_4 <= 0.5:
                                if race_3 <= 0.5:
                                    if c_charge_degree_2 <= 0.5:
                                        if age_cat_1 <= 0.5:
                                            if length_of_stay_4 <= 0.5:
                                                if length_of_stay_5 <= 0.5:
                                                    if race_1 <= 0.5:
                                                        if length_of_stay_3 <= 0.5:
                                                            if score_text_2 <= 0.5:

                                                                 return 0
                                                            else:  # if score_text_2 > 0.5

                                                                 return 0
                                                        else:  # if length_of_stay_3 > 0.5
                                                            if score_text_2 <= 0.5:
                                                        ...
                                                        if length_of_stay_1 <= 0.5:
                                                            if score_text_1 <= 0.5:

                                                                 return 0
                                                            else:  # if score_text_1 > 0.5

                                                                 return 0
                                                        else:  # if length_of_stay_1 > 0.5
                                                            if score_text_2 <= 0.5:
                                    ...
                                    if score_text_1 <= 0.5:
                                        ...
                                        if age_cat_2 <= 0.5:
                                            if length_of_stay_5 <= 0.5:
                                                if c_charge_degree_1 <= 0.5:

                                                     return 0
                                                else:  # if c_charge_degree_1 > 0.5
                                                    if length_of_stay_3 <= 0.5:
